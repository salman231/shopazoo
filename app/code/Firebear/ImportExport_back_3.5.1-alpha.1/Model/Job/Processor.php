...
class Processor
{
    ...
    public function processScope($jobId, $file, $offset = 1)
    {
        ...
        try {
            ...
            if ($this->strategy == 'validation-skip-errors') {
                ...
                $result = !$importModel->getErrorAggregator()->isErrorLimitExceeded();
            }
        } catch (\Exception $e) {
            $this->addLogComment(
    ...
    public function prepareJob($jobId)
    {
        ...
        $this->job = $this->jobRepository->getById($jobId);
        $data = [];
        if ($this->job->getId()) {
            $behaviorData = $this->job->getBehaviorData();
            $sourceData = $this->job->getSourceData();
    ...
    public function processImport($file, $job, $offset, $error, $show = 1)
    {
        ...
        return [
            (int) $this->importModel->getErrorAggregator()->isErrorLimitExceeded(),
        ...
        if ($this->reindex) {
            try {
                ...
                if (!empty($this->indexers)) {
                    $this->reindexIndexer($this->indexers);
                } else {
                    $this->reindexAll();
                }
            } catch (\Exception $e) {
     ...
     * @param $indexers
     */
    protected function reindexIndexer(array $indexers)
    {
        $this->addLogComment(__('Running REINDEX'), $this->output, 'info');
        foreach ($indexers as $indexerName) {
            $index = $this->indexerRegistry->get($indexerName);
            $this->addLogComment(__('REINDEX %1', $index->getTitle()), $this->output, 'info');
            if (!$index->isScheduled()) {
                $index->reindexRow($index->getId());
            }
        }
        $this->addLogComment(__('REINDEX completed'), $this->output, 'info');
    }
    ...
    public function getColumns($job)
    {
        ...
        if ($data['import_source'] == 'file') {
            ...
            try {
                ...
                if (is_string($errorMessage) && strpos($errorMessage, 'ftp_get()') !== false) {
