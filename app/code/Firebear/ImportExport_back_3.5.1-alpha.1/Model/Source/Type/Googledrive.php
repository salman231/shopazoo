...
class Googledrive extends AbstractType
{
    ...
    public function importImage($importImage, $imageSting)
    {
        if ($client = $this->_getSourceClient()) {
            if (preg_match('/\bhttps?:\/\//i', $importImage, $matches)) {
                ...
                $googleDriveFileDirectory = dirname($sourceFilePath);
                ...
                if ($sourceDir !== '') {
                    $importImage = $sourceDir . '/' . $importImage;
                }

                if ($googleDriveFileDirectory !== '.') {
                    $importImage = $googleDriveFileDirectory . '/' . $importImage;
                }
                ...
                try {
                    if ($image) {
                        ...
                        $service = $this->_getSourceService();
                        $fileMetadata = $service->files->get($image->id, [
                            'alt' => 'media']);
                        file_put_contents($filePath, $fileMetadata->getBody()->getContents());
                    }
                } catch (\Exception $e) {
    ...
    protected function _getFileByPath($path)
    {
        ...
        $fileName = array_pop($filePathInfo);
