...
class OrderInfoBuilder implements OrderInfoBuilderInterface
{
        ...
        if ($shipmentCollection->getSize()) {
            foreach ($shipmentCollection->getItems() as $shipment) {
                ...
                foreach ($sItems as $shipmentItem) {
                    ...
                    $shipmentTracks[$shipmentItem['parent_id']]['inventory_source'] =
                        $inventorySourcesByItemIds[$shipmentItem['parent_id']];
    ...
    protected function mapFieldsForWesupplyStructure($orderData, $existingOrderData)
    {
        ...
        if (count($orderItems) == 0) {
            /**
             * if we only have virtual or downloadable products in order
             * we are not updating wesupply_orders table
             */
            return false;
        }
    ...
    protected function prepareOrderItems($orderData, $existingOrderData)
    {
        ...
        foreach ($orderItemsData as $item) {
            ...
            $itemQtyGrouped = $this->splitItemQty($item);
            ...
            if ($itemQtyGrouped['qty_canceled'] > 0 && !$addedToShipment) {
                $itemInfo = $this->getItemSpecificInformation(
                    $itemFeeShipping,
                    $itemTotals['row_total'],
                    $itemTotals['tax_amount'],
    ...
    protected function _fetchInvariableData($existingOrderData, $item, &$generalData)
    {
        ...
        if ($existingOrderData) {// check if this is the first sync or it is an update
            if ($this->_isMultiProducts($existingOrderData['OrderItems']['Item'])) {
                $found = array_filter(
                    $existingItemData = $existingOrderData['OrderItems']['Item'],
            ...
            foreach ($shipments->getItems() as $shipment) {
                ...
                foreach ($shipmentDetails->getItems() as $item) {
                    if (!method_exists($extensionAttr, 'getSourceCode')) {
                        $this->logger->error('Cannot fetch Magento MSI details. Check to make sure the functionality is enabled and properly configured.');
     ...
     * @return array
     */
    private function splitItemQty($item)
    {
        $qtySplit = [];
        $qtySplit['qty_ordered']    = intval($this->_helper->recursivelyGetArrayData(['qty_ordered'], $item));
        $qtySplit['qty_canceled']   = intval($this->_helper->recursivelyGetArrayData(['qty_canceled'], $item, 0));
        $qtySplit['qty_refunded']   = intval($this->_helper->recursivelyGetArrayData(['qty_refunded'], $item, 0));
        $qtySplit['qty_shipped']    = intval($this->_helper->recursivelyGetArrayData(['qty_shipped'], $item, 0));
        $qtySplit['qty_processing'] = $qtySplit['qty_ordered'] - $qtySplit['qty_canceled'] - $qtySplit['qty_refunded'] - $qtySplit['qty_shipped'];
